...
class AppContext {
	...
	private void doAutowired() {
		...
		try {
			for(Object bean : map.values()) {
				...
				//특정 클래스에 선언된 멤버변수를 동적으로 읽거나 수정하는 데 사용된다.
	...
	private void doComponentScan() {
		try {
			...
			// 2. 반복문으로 클래스를 하나씩 읽어서 @Component가 붙어있는지 확인한다.
			// 3. @Component가 붙어있으면 객체를 생성해서 map에 저장한다.
...
//		car.engine = new Engine(); // 이렇게 새로 만들지 않고
//		car.engine = engine; // 만들어서 저장해놓은 객체를 주겠다.
//		car.door = door;
		//따라서 출력 결과의 주소를 살펴보면 엔진 객체와 도어 객체 각각 주소가 같은 것을 볼 수 있다.
