...
public class TodoService {
	...
	public List<TodoEntity> create(TodoEntity entity){
		...
		//{}는 Slf4j에서 제공하는 플레이스홀더로, 두 번째 매개변수로 전달된 entity.getId()값이 대잆되어 출력된다.
		log.info("Entity Id : {} is saved",entity.getId());

		//넘어온 entity로부터 userId를 가지고 DB에서 조회된 내용을 List에 묶어서 반환
		//SELECT * FROM TodoEntity where userid = 'temporaray-user';
		//실행해서 조회된 결과를 List에 묶어서 봔한하겠다.
...
//		})
	...
	public List<TodoEntity> delete(TodoEntity entity){		
		...
		try {
			...
			log.error("Entity deleting entity",entity.getId(),e);
			//컨트롤러로 exception을 날린다.
			//데이터베이스 내부 로직을 캡슐과 하기 위해 e를 반환하지 않고 새 exception 객체를 반환한다.
