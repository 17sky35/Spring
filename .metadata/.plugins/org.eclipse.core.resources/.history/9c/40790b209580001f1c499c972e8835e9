...
@RequestMapping("todo")
@RequiredArgsConstructor 	//final 이나 @NonNull이 붙어있는 필드만 생성자의 매개변수로 생성
public class TodoController {
	...
	//testTodo에서는 ResponseEntity.ok().body(response)로 반환하기
	...
	public ResponseEntity<?> createTodo(@RequestBody TodoDTO dto){
		try {
			String temporarUserId = "temporary-user"; //임시 유저 id
			
			//TodoDTO객체를 TodoEntity로 변환한다.
			TodoEntity entity = TodoDTO.toEntity(dto);
			
			//id에 null이 이미 들어가있지만 명시적으로 null로 한번 더 설정한다.
			//해당 엔티티가 새로운 데이터임을 보장하게된다.
			entity.setId(null);
			
			//임시 userId 설정.
			//지금은 인증과 인가 기능이 없으므로 임시 유저 (temporary-user)만 로그인 없이 사용 가능한 애플리케이션인셈이다.
			...
			//entities.stream().map(TodoDTO::new) : TodoEntity 객체들을 TodoDTO객체들로 변환하는과정
			//.collect(Collectors.toList()); : Stream으로 반환된 객체들을 List로 반환
			List<TodoDTO> dtos = entities.stream().map(TodoDTO::new).collect(Collectors.toList());
			
			//변환된 TodoDTO객체를 담고있는 리스트를 ResponseDTO의 data필드에 대입한다.
			ResponseDTO<TodoDTO> response = ResponseDTO.<TodoDTO>builder().data(dtos).build();
						
			//responseDTO객체를 돌려줘야됨
			return  ResponseEntity.ok().body(response);
	...
	public ResponseEntity<?> updateTodo(@RequestBody TodoDTO dto){
		...
		//dto에는 userId에 대한 정보가 없기 떄문에 임시유저를 묶어서 보내야한다.
